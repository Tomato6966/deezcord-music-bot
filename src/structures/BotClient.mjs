import { Client, GatewayIntentBits, Partials, ActivityType, PresenceUpdateStatus, Collection, Options, SlashCommandAssertions, PermissionsBitField, PermissionFlagsBits, ChannelType, SlashCommandBuilder } from "discord.js";
...
export class BotClient extends Client {
    ...
    async loadCommads(path="/src/commands") {
        ...
        for(const dir of dirs) {
            ...
            if ((await promises.lstat(`${process.cwd()}${path}/${dir}/`).catch(console.error))?.isDirectory?.()) {
                    ...
                    if ((await promises.lstat(curPath).catch(console.error))?.isDirectory?.()) {
                        ...
                        if (slashCommands?.length) {
                            ...
                            for (let sFile of slashCommands) {
                                ...
                                commands.push(await import(groupCurPath).then(x => x.default));
                            }
                ...
                this.buildOptions(command, Slash);
                command.commandId = cache?.fetchedApplication?.find?.(c => c?.name == command.name)?.permissions?.commandId ?? "commandId";
    ...
    async publishCommands() {
        ...
        await this.application.commands.set(this.allCommands)
        .then(() => {
